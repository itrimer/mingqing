<?php
class Captcha_show extends CI_Controller {
	...
	function refresh($t = null) {
		$this->rand_create();
	}
	private function rand_create() {
		session_start ();
		...
		$cap_data[$this->config->item ( 'captcha_code' )] =
			md5 ( $this->config->item ( 'captcha_pre' )
				. $verifyCode
				. $this->config->item ( 'captcha_end' ));
		$this->session->set_userdata($cap_data);
		
// 		$_SESSION [$this->config->item ( 'captcha_code' )] = 
// 			md5 ( $this->config->item ( 'captcha_pre' ) 
// 				. $verifyCode 
// 				. $this->config->item ( 'captcha_end' ));
