...
<div id="page-wrapper">
	...
	<div id="main-wrapper">
		<div id="main-content">
	 		<div class="path">当前位置：系统网盘  &gt; <a href="<?php echo base_url()?>admin/share/">私人空间</a>
	 			<span><?php $tree_i = 0;
	 				foreach($tree_array as $key => $row) { $tree_i ++;
						if($tree_i < count($tree_array)){
	 				?>
	 				&gt; <a href="<?php echo base_url()?>admin/share/?path=<?php echo $row['share_id'];?>"><?php echo $row['share_name'];?></a>
	 				<?php } else {
	 					echo '&gt; '.$row['share_name'];
  		   			}
					}?>
			...
			<div class="bar_block">
				<div class="float-left">
					<form name="formSearch" action="." method="get">
						<input name="path" type="hidden" value="<?php echo $path?$path:'0';?>"/>
			...
			<form name="myform" class="pager-form" method="post" action="">
				...
				<thead>
					<tr>
						<td width="10"><input type="checkbox" onclick="check_rows(this, this.form.ids)" /></td>
						<td>文件名</td>
...
?>			
				<tr height="30" class="<?php echo $i % 2 == 0?'even':'odd'?>">
					<td>
						<input name="ids" type="checkbox" value="<?php echo $row["share_id"]?>" />
					</td>
					<td>
					<?php if($row['is_directory'] == 1){?> 
						<span class="folder"></span> 
						<a href="?path=<?php echo $row["share_id"]?>" title="进入文件夹"><?php echo $row["share_name"]?></a>
					<?php } else {?>
						<?php echo $row["share_name"]?>
					<?php }?>
					</td>
					<td><?php echo $row["file_size"]?$row["file_size"]:'0'?></td>
					<td><?php echo $row["update_date"]?></td>
					<td><?php echo $row["is_shared"] == 1?'是':'否'?></td>
			   	</tr>
<?php
}
?>
...
$(function(){
	$('#dir_add').click(function(){
		var params = {width:350, height:200};
		params.template_url = base_url_index + "admin/share/mkdir?parent_id=<?php echo $path;?>";
		params.success_text = "新建成功";
		do_template(params);
		return false;
	});
	$('#share_add').click(function(){
		var params = {width:400, height:200};
		params.template_url = base_url_index + "admin/share/upload?parent_id=<?php echo $path;?>";
		params.success_text = "上传成功";
		do_template(params);
		return false;
	});
